...
def parse_args():
    parser = argparse.ArgumentParser(description="RNA-seq Expression Matrix Preprocessing, QC, and Top-MAD Feature Selection.")
    ...
    parser.add_argument("--mad_top_fraction", type=float, default=0.25, help="Fraction of genes with highest MAD to retain (e.g., 0.25 for top 25%).")
...
def main():
    ...
    # Robustness check for fraction parameter (must be between 0 and 1)
    if not (0 < args.mad_top_fraction <= 1):
        raise ValueError("--mad_top_fraction must be > 0 and <= 1 (e.g. 0.25 for top 25%).")
    ...
    removed_samples_library = original_n_samples - after_qc_n_samples
    print(f"Samples: {original_n_samples:,} -> {after_qc_n_samples:,} (removed {removed_samples_library:,})")
    ...
    removed_genes_annotation = original_n_genes - annotation_n_genes
    ...
    removed_genes_length = annotation_n_genes - valid_length_n_genes
    ...
    removed_genes_cpm = valid_length_n_genes - after_cpm_n_genes
    print(f"Genes after CPM filter: {valid_length_n_genes:,} -> {after_cpm_n_genes:,} (removed {removed_genes_cpm:,})")
    ...
    # =========================================================
    # Step 7: High-Variability Gene Selection (Top Fraction MAD Filtering)
    # =========================================================
    print(f">>> Step 7: High-Variability Gene Selection (Top {args.mad_top_fraction*100:.1f}% MAD filtering)...")
    gene_median = filtered_log_tpm.median(axis=1)
    mad = filtered_log_tpm.sub(gene_median, axis=0).abs().median(axis=1)
    
    # Calculate top fraction cutoff using nlargest
    n_keep = int(np.ceil(len(mad) * args.mad_top_fraction))
    top_variable_genes = mad.nlargest(n_keep).index
    
    # Calculate and print the actual MAD cutoff for this fraction
    mad_cutoff = mad.loc[top_variable_genes].min()
    print(f"MAD cutoff for top {args.mad_top_fraction*100:.1f}%: {mad_cutoff:.4f}")
    
    final_log_tpm = filtered_log_tpm.loc[top_variable_genes]
    final_n_genes = final_log_tpm.shape[0]
    removed_genes_mad = after_cpm_n_genes - final_n_genes
    
    # Save gene-wise MAD statistics sorted by MAD descending with rank
    mad_df = pd.DataFrame({
        "MAD": mad,
        "MAD_rank": mad.rank(method="min", ascending=False),
        "selected_top_mad": mad.index.isin(top_variable_genes)
    })
    mad_df = mad_df.sort_values("MAD", ascending=False)
    mad_df.to_csv(os.path.join(args.out_dir, "gene_mad_statistics.csv"))
        
    print(f"Genes after Top {args.mad_top_fraction*100:.1f}% MAD filter: {after_cpm_n_genes:,} -> {final_n_genes:,} (removed {removed_genes_mad:,})")
    ...
    print(f"  Top {args.mad_top_fraction*100:.1f}% MAD filtering:     {final_n_genes:,}")
    print("===========================================\n")
    ...
    summary = pd.DataFrame({
        "step": [
            ...
            "Valid gene length",
            "CPM filtering",
            f"Top {args.mad_top_fraction*100:.1f}% MAD filtering"
        ],
        "n_samples": [
            original_n_samples,
            after_qc_n_samples,
        ...
        "removed_genes": [
            0,
            0,
            removed_genes_annotation,
            removed_genes_length,
            removed_genes_cpm,
            removed_genes_mad
        ]
    })
